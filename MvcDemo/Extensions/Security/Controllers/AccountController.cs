...
namespace Demo.Extensions.Security.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        {
            if (await _userManager.UpdateAsync(UserId, new { model.Email }))
            {
            ...
            var result =
                await
                    _userManager.ChangePasswordAsync(user,
                        _userManager.PasswordSalt(user.UserName, model.Password),
                        _userManager.PasswordSalt(user.UserName, model.NewPassword));
            if (result.Succeeded)
            {
