...
namespace Mozlite.Extensions.Security.Stores
{
    ...
    public class UserStoreEx<TUser, TRole, TUserClaim, TUserRole, TUserLogin, TUserToken, TRoleClaim>
        : UserStoreExBase<TUser, TRole, TUserClaim, TUserRole, TUserLogin, TUserToken, TRoleClaim>
        ...
        where TRoleClaim : RoleClaimExBase, new()
    {
        ...
        public UserStoreEx(IdentityErrorDescriber describer,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await UserLoginContext.DeleteAsync(
                x => x.UserId == user.UserId && x.SiteId == Site.SiteId && x.LoginProvider == loginProvider && x.ProviderKey == providerKey,
                cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return await UserRoleContext.AnyAsync(x => x.UserId == user.UserId && x.RoleId == role.RoleId && x.SiteId == Site.SiteId, cancellationToken);
        }
        ...
        {
            return UserRoleContext.FindAsync(x => x.UserId == userId && x.SiteId == Site.SiteId && x.RoleId == roleId, cancellationToken);
        }
        ...
        public override async Task<IList<TUser>> GetUsersInRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var users = await UserContext.AsQueryable().InnerJoin<TUserRole>((u, l) => u.UserId == l.UserId)
                .Where<TUserRole>(x => x.RoleId == role.RoleId && x.SiteId == Site.SiteId)
                .Where(x => x.SiteId == Site.SiteId)
                .AsEnumerableAsync(cancellationToken);
            return users.ToList();
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            var role = await FindRoleAsync(normalizedRoleName, cancellationToken);
            if (role != null)
                await UserRoleContext.DeleteAsync(x => x.UserId == user.UserId && x.RoleId == role.RoleId && x.SiteId == Site.SiteId, cancellationToken);
        }
        ...
        {
            var userRoles = await UserRoleContext.FetchAsync(x => x.UserId == user.UserId && x.SiteId == Site.SiteId, cancellationToken);
        ...
        /// 缓存键。
        /// </summary>
        protected Tuple<int, Type> CacheKey => new Tuple<int, Type>(Site.SiteId, typeof(TRole));
        ...
        {
            return await _cache.GetOrCreateAsync(CacheKey, async ctx =>
            {
