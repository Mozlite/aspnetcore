...
namespace Mozlite.Extensions.Security.Stores
{
    ...
    public class RoleStoreEx<TRole, TUserRole, TRoleClaim> : RoleStoreExBase<TRole, TUserRole, TRoleClaim>
        ...
        where TRoleClaim : RoleClaimExBase, new()
    {
        ...
        /// <summary>
        /// 初始化类<see cref="RoleStoreEx{TRole,TUserRole,TRoleClaim}"/>。
        /// </summary>
        ...
        public RoleStoreEx(IdentityErrorDescriber describer,
        ...
        public override async Task<TRole> FindByIdAsync(string id, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return roles.SingleOrDefault(x => x.RoleId == roleId && x.SiteId == Site.SiteId);
        }
        ...
        {
            var roles = await LoadRolesAsync();
            return roles.SingleOrDefault(x => x.NormalizedName == normalizedName && x.SiteId == Site.SiteId);
        }
        ...
        public override async Task<IList<Claim>> GetClaimsAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var claims = await RoleClaimContext.FetchAsync(x => x.RoleId == role.RoleId && x.SiteId == Site.SiteId, cancellationToken);
        ...
        public override async Task AddClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var roleClaim = new TRoleClaim { RoleId = role.RoleId, SiteId = Site.SiteId };
        ...
        public override async Task RemoveClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await RoleClaimContext.DeleteAsync(x =>
                x.RoleId == role.RoleId && x.SiteId == Site.SiteId && x.ClaimType == claim.Type && x.ClaimValue == claim.Value, cancellationToken);
        }
        ...
        /// 获取当前角色可查询实例。
        /// </summary>
        public override System.Linq.IQueryable<TRole> Roles => Cache.GetOrCreate(CacheKey, ctx =>
        {
            ctx.SetDefaultAbsoluteExpiration();
