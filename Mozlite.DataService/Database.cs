...
namespace Mozlite.DataService
{
    ...
    public abstract class Database : IDatabase
    {
        ...
        public virtual bool BeginTransaction(Func<IDbTransaction, bool> executor, int timeout = 30)
        {
            using (var connection = GetConnection())
            {
                connection.Open();
        ...
        /// <returns>返回事务实例对象。</returns>
        public virtual async Task<bool> BeginTransactionAsync(Func<IDbTransaction, Task<bool>> executor, int timeout = 30, CancellationToken cancellationToken = default)
        {
            using (var connection = GetConnection())
            {
        ...
        class Transaction : IDbTransaction
        {
